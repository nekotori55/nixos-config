...
in
{
  options.services.zapret-custom = {
    ...
    lib.mkMerge [
      ...
      (lib.mkIf cfg.configureFirewall {
        networking.firewall.extraCommands =
          ...
          in
          ''
            ip46tables -t mangle -N ZAPRET_MARK

            ip46tables -t mangle -A ZAPRET_MARK -p tcp -m set --match-set ZAPRET_IPS dst -j MARK --set-mark 1
            ip46tables -t mangle -A ZAPRET_MARK -p udp -m set --match-set ZAPRET_IPS dst -j MARK --set-mark 1

            ip46tables -t mangle -A PREROUTING -i tun0 -j ZAPRET_MARK

            ip46tables -t mangle -A OUTPUT -j ZAPRET_MARK

            ip46tables -t mangle -A PREROUTING -m mark --mark 1 -m mark ! --mark 0x40000000/0x40000000 -j NFQUEUE --queue-num 0 --queue-bypass
          '';
        # +
        #   # mark every tun0 packet
        #   ''
        #     ip46tables -t mangle -A PREROUTING -i tun0 -j MARK --set-mark 1
        #   ''
        # + ''
        ...
        # ''
        # # send all marked packets to nfqws process
        # + ''
        #   ip46tables -t mangle -A PREROUTING -m mark --mark 1 -j NFQUEUE --queue-num 0 --queue-bypass
        # '';
      })
    ]
  );
